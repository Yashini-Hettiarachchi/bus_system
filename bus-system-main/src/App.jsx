...
const GOOGLE_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbzKNi7AG_O8hWqZvVqwHPyr83C4IKbROT-i-vT51YAKdc51eqZVIzsWqa8ZzhRFvVmt-g/exec";

async function sendAnswerToSheet({ destination, companion }) {
  const payload = {
...
function App() {
  ...
  const [destinationAnswer, setDestinationAnswer] = useState("");
  // Show 'game' name only when news section is scrolled
  useEffect(() => {
    if (tab !== "news") return;
        ...
        >
          {TAB_OPTIONS.map((option, idx) => (
            <>
              ...
              {/* Insert game icon after movie tab, only while scrolling news */}
              {option.id === "movie" &&
                tab === "news" &&
                showGameName &&
                (!showBus ? (
          ...
          {showBus && (
            ...
            >
              ...
              >
                <span
                  role="img"
                  aria-label={showSecondQuestion ? "companion" : "bus stop"}
                  style={{
                    fontSize: 64,
                    display: "inline-block",
                ...
                {!showSecondQuestion ? (
                  <>
                    ...
                    <form
                      onSubmit={async (e) => {
                        e.preventDefault();
                        if (!destinationAnswer.trim()) return;
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => setDestinationAnswer(e.target.value)}
                        style={{
                          marginTop: 4,
                          padding: 8,
                          fontSize: 16,
                      ...
                      <button
                        ...
                        disabled={!destinationAnswer.trim()}
                      >
                        Next
                      </button>
                    </form>
                  ...
                  <>
                    ...
                    >
                      ඔබ සමඟ සිටින පුද්ගලයා කවුද? <span role="img" aria-label="companion">🫂</span>
                    </span>
                    <form
                      onSubmit={async (e) => {
                        e.preventDefault();
                        if (!companionAnswer.trim()) return;
                        await sendAnswerToSheet({
                          destination: destinationAnswer.trim(),
                          companion: companionAnswer.trim(),
                        });
                        setDestinationAnswer("");
                        setCompanionAnswer("");
                        setShowSecondQuestion(false);
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => setCompanionAnswer(e.target.value)}
                        style={{
                          marginTop: 4,
                          padding: 8,
                          fontSize: 16,
